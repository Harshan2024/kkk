    ...
    <div className="space-y-6 select-none relative">
      ...
      <AnimatePresence>
        {toast && (
          ...
          >
            <div className={`glass-premium rounded-2xl p-4 border flex items-start gap-3 shadow-xl ${
              toast.type === "success" 
                ? "border-emerald-500/30 bg-emerald-500/10 shadow-emerald-500/5 text-theme-brand" 
      ...
      <div className="glass-premium rounded-3xl p-6 border border-emerald-500/10 bg-gradient-to-br from-emerald-950/5 via-theme-surface to-theme-base relative overflow-hidden flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center space-x-3.5">
          ...
          <div>
            <h1 className="text-lg font-black tracking-tight text-theme-heading flex items-center gap-1.5 font-display">
              Virtual Rewards Shop <Badge variant="success" size="xs">XP Redemption</Badge>
            </h1>
        ...
        {profile && (
          <div className="flex items-center gap-3 bg-theme-base border border-theme-subtle px-4 py-2.5 rounded-2xl">
            <Zap className="w-5 h-5 text-amber-400 fill-amber-400 animate-pulse" />
            <div>
              <span className="text-[8px] font-black text-theme-muted uppercase tracking-wider block">Available Balance</span>
              <span className="text-sm font-black text-white">{profile.available_xp} <span className="text-[10px] text-theme-muted font-bold">XP</span></span>
            </div>
          </div>
        )}
      </div>
      ...
      {isLoading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[1, 2, 3, 4].map(idx => (
            <div key={idx} className="glass-premium rounded-2xl p-5 h-[160px] animate-pulse" />
          ))}
        </div>
      ) : safeRewards.length === 0 ? (
        <div className="w-full py-12 text-center border border-dashed border-theme-subtle rounded-3xl bg-white/[0.01]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {(Array.isArray(rewards) ? rewards : []).map(item => {
            ...
            return (
              <div 
                key={item.id}
                className={`glass-premium rounded-2xl p-5 border transition-all duration-300 flex flex-col justify-between h-[160px] ${
                  item.redeemed 
                    ? "bg-emerald-950/[0.03] border-emerald-500/15" 
                    : "bg-white/[0.01] border-theme-subtle hover:border-emerald-500/10"
                }`}
              >
                <div className="flex items-start gap-4">
                  <div className={`w-10 h-10 rounded-xl border flex items-center justify-center flex-shrink-0 ${
                    item.redeemed 
                      ...
                      : "text-theme-muted bg-white/5 border-white/5"
                  }`}>
                    <Icon className="w-5 h-5" />
                  </div>
                  <div>
                    <h3 className="text-xs font-black text-theme-primary uppercase tracking-wider">{item.name}</h3>
                    <p className="text-[10px] text-theme-muted font-bold leading-relaxed mt-1">{item.description}</p>
                  </div>
                </div>

                <div className="flex items-center justify-between border-t border-theme-subtle pt-3 mt-3">
                  ...
                  {item.redeemed ? (
                    <span className="text-[9px] font-black uppercase text-theme-brand bg-emerald-500/15 border border-emerald-500/20 px-3 py-1.5 rounded-xl">
                      Redeemed ✓
                    </span>
                  ) : (
                    <Button
